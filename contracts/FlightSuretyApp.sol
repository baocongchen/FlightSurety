...
contract FlightSuretyApp {
    ...
    event InsurancePurchased(address passenger, address airline, string flight, uint256 timestamp);
    /********************************************************************************************/
    /*                                       FUNCTION MODIFIERS                                 */
    /********************************************************************************************/
    ...
    */
    ...
    modifier requireAirlineRegistered() {
        require(flightSuretyData.getAirlineRegistrationStatus(msg.sender) == true, "Airline is not registered");
        _;
    }

    modifier requireAirlinePaidFund() {
        require(flightSuretyData.getAirlineFundingStatus(msg.sender) == true, "Fund not paid");
        _;
    }

    modifier requireHasNotVoted( address airlineToBeVoted ) {
        require(flightSuretyData.getAirlineVotingStatus(msg.sender, airlineToBeVoted) == false, "Cannot vote twice for a single airline");
        _;
    }

    modifier requireHasNotFunded() {
        require(flightSuretyData.getAirlineFundingStatus(msg.sender) == false, "Cannot vote twice for a single airline");
        _;
    }
    /********************************************************************************************/
    /*                                       CONSTRUCTOR                                        */
    ...
    */   
                            ...
                            returns(bool)   
    {
        if (registeredAirlines < 4) {
            require(flightSuretyData.getAirlineFundingStatus(msg.sender), "Unable to register/vote for other airlines unless fund is paid");
            ...
            if (votesEarned[airline].mul(100).div(registeredAirlines) >= CONSENSUS) {
    ...
    */  
    ...
    function buyInsurance(
        ...
        require(isFlightRegistered(airline, flight, timestamp), "Flight is not registered");
...
// region ORACLE MANAGEMENT
                            ...
                            requireIsOperational
    {
        ...
        require(!oracles[msg.sender].isRegistered, "Unable to register! Oracle already exists");
                            ...
                            returns (uint256)
    {
        ...
        uint256 random = uint256(uint256(keccak256(abi.encodePacked(blockhash(block.number - nonce++), account))) % maxValue);
...
contract FlightSuretyData {
    ...
    function getAirlineRegistrationStatus(address airline) external view returns(bool);

    function getAirlineVotingStatus(address airline, address airlineToBeVoted) external view returns(bool);
