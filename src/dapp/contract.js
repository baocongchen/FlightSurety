...
export default class Contract {
    ...
    registerAirline(airline, registrar, callback) {
        const self = this;
        self.flightSuretyApp.methods
                            .registerAirline(airline).send({from: registrar}, (error, result) => {
                                if (error === null) {
                                    callback(error, `Airline ${airline} has been registered`);
                                } else {
                                    callback(`Airline ${airline} has not been registered due to error ${error}`);
                                }
                            });
    }
    ...
    fundAirline(airline, callback) {
        ...
        self.flightSuretyApp.methods
            .fundAirline()
            .send({from: airline, value: amount, gas:self.gas}, (error, result) => {
        ...
        self.flightSuretyApp.methods
            ...
            .send({from: airline, gas: self.gas},(error, result) => {
                if (error) {
                    ...
                    console.log(result);
                    callback(error, `Airline ${  airline  } successfully registered flight ${  flight  } that will fly at ${  timestamp }`)
                }
            });
    }
    ...
    buyInsurance(airline, flight, timestamp, passenger, value, callback) {
        ...
        self.flightSuretyApp.methods
            ...
            .send({from: passenger, value: amount, gas: self.gas}, (error, result) => {
                if (error) {
                    console.log(error, payload);
                    callback(error, payload);
                } else {
                    console.log(result);
                    callback(error, `Passenger ${  passenger  } successfully purchased flight insurance valued at ${  amount  }`)
                }
        });
    }
